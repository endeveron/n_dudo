...
export function isSessionStale(
  ...
  // 1. Check if session is close to expiring
  if (session.expires) {
  ...
  // 2. Check if access token is close to expiring (if you have this data)
  ...
  // 3. Check if session is too old (based on when token was issued)
  if (session.iat) {
